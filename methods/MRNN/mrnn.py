...
import numpy as np
from methods.MRNN.model_utils import biGRUCell, initial_point_interpolation
...
class mrnn ():
  ...
  def rnn_train (self, x, m, t, f):  
    ...
    with tf.compat.v1.Session() as sess:        
      ...
      start_save = 'tmp\\mrnn_imputation\\rnn_feature_' + str(f+1) + '\\'
      current_directory = os.getcwd()
      save_file_name = os.path.join(current_directory, start_save)
  ...
  def fc_train(self, x, m, t):
    ...
    start_save = 'tmp\\mrnn_imputation\\fc_feature\\'
    current_directory = os.getcwd()
    save_file_name = os.path.join(current_directory, start_save)    
    tf.compat.v1.saved_model.simple_save(sess, save_file_name, 
                                         inputs, outputs)  
